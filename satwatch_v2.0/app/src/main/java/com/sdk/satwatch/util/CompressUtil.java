...
public class CompressUtil {
    ...
    public static void compressToZip(File sourceFile, String targetPath) {
        ...
        //2.使用CRC32进行文件校验
        LogUtil.info("start to compress file to zip, file name:{}", sourceFile.getName());
        ...
        try (FileOutputStream fileOut = new FileOutputStream(targetFile);
            ...
            LogUtil.error("compress file to zip throw exception:{}", e.toString());
        } catch (IOException e) {
            LogUtil.error("compress file to zip throw exception:{}", e.toString());
        }
//        LogUtil.info("finish compress file to zip, file name:{}, cost:{} ms", sourceFile.getName(), System.currentTimeMillis() - start);
    }
    ...
    public static void compressToTarGz(String sourcePath, String targetPath) {
        ...
        LogUtil.info("start to compress file to tar.gz, file name:{}", sourceFile.getName());
        long start = System.currentTimeMillis();
        //1.压缩为tar
        ...
        if (!FileUtil.deleteFile(rarSourcePath)) {
            LogUtil.error("delete rar file field, rar file path:{}", rarSourcePath);
        }

        LogUtil.info("finish compress file to tar.gz, file name:{}, cost:{} ms", sourceFile.getName() + "" + (System.currentTimeMillis() - start));
    }
    ...
    public static void compressTarToGz(File sourceFile, String targetPath) {
        //校验解压路径是否存在
        FileUtil.validateTargetPath(targetPath);
        LogUtil.info("start to compress tar file to tar.gz, file name:{}", sourceFile.getName());
        long start = System.currentTimeMillis();
        try (BufferedInputStream bis = new BufferedInputStream(new FileInputStream(sourceFile))) {
        ...
        LogUtil.info("finish compress tar file to tar.gz, file name:{}, cost:{} ms", sourceFile.getName() + " " + (System.currentTimeMillis() - start));
    }
    ...
    public static String compressToTar(String sourcePath, String targetPath) {
        ...
        LogUtil.info("start compress file to tar, file name:{}, cost:{} ms", sourceFile.getName());
        long start = System.currentTimeMillis();
        ...
        LogUtil.info("finish compress file to tar, file name:{}, cost:{} ms", sourceFile.getName() + " " + (System.currentTimeMillis() - start));
        //返回tar压缩文件路径
