...
public class RuleView extends View {
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        baseLineColor = ta.getColor(R.styleable.RuleView_zjun_baseLineColor, Color.LTGRAY);
        longGradationColor = ta.getColor(R.styleable.RuleView_zjun_longGradationColor, Color.LTGRAY);
        shortGradationColor = ta.getColor(R.styleable.RuleView_zjun_shortGradationColor, Color.LTGRAY);
        ...
        bgRadius = ta.getDimension(R.styleable.RuleView_zjun_radius, dp2px(0));
...
//     * @see ViewGroup#getChildMeasureSpec(int, int, int)
    ...
    private void scrollToGradation() {
        ...
        mCurrentDistance = (mCurrentNumber - mMinNumber) / mNumberUnit * gradationGap;
        currentValue = mCurrentNumber / 10f;
//        logD("scrollToGradation: mCurrentDistance=%f, mCurrentNumber=%d, currentValue=%f, round=%d, Gap=%f, Unit=%d",
//                mCurrentDistance, mCurrentNumber, currentValue, Math.round(mCurrentDistance / gradationGap), (mCurrentDistance / gradationGap), mNumberUnit);
        if (mValueChangedListener != null) {
            mValueChangedListener.onValueChanged(currentValue);
        }
        invalidate();
...
//        canvas.drawColor(bgColor);
        ...
        canvas.drawRoundRect(rectF, bgRadius, bgRadius, mPaint);
        // 2 绘制刻度、数字
    ...
    private void drawGradation(Canvas canvas) {
        ...
        logD("drawGradation: startNum=%d, rightNum=%d, perUnitCount=%d,  mNumberUnit=%d",
                startNum, rightMaxNum, perUnitCount, mNumberUnit);
        while (startNum <= rightMaxNum) {
            logD("drawGradation: startNum=%d", startNum);
            if (startNum % perUnitCount == 0) {
                // 长刻度：刻度宽度为短刻度的2倍
                mPaint.setColor(longGradationColor);
                mPaint.setStrokeWidth(longLineWidth);
                canvas.drawLine(distance, mHeight, distance, mHeight - longGradationLen, mPaint);
                ...
                canvas.drawText(text, distance - textWidth * .5f, mHeight - textSize - gradationNumberGap, mTextPaint);
            } else {
                // 短刻度
                mPaint.setColor(shortGradationColor);
                mPaint.setStrokeWidth(shortLineWidth);
                canvas.drawLine(distance, mHeight, distance, mHeight - shortGradationLen, mPaint);
            }
        ...
        mPaint.setStrokeWidth(shortLineWidth);
        canvas.drawLine(bgRadius, mHeight/*shortLineWidth * .5f*/, mWidth - bgRadius, mHeight, mPaint);
    }
    ...
    private void drawIndicator(Canvas canvas) {
        ...
        //
        canvas.drawLine(mHalfWidth, mHeight, mHalfWidth, mHeight - indicatorLineLen, mPaint);
        // 默认形状画笔
    ...
    public void setValue(float minValue, float maxValue, float curValue, float unit, int perCount) {
        ...
        postInvalidate();
        Log.i(""," gradationUnit == " + gradationUnit + " numberPerCount == " + numberPerCount);
    }
