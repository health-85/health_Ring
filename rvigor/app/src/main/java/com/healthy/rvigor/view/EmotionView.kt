...
class EmotionView : View {
    ...
    private fun drawHorizontalLine(canvas: Canvas) {
        ...
        for (i in 0..len.toInt()) {
            if (i == 1){
                touchTxt = resources.getString(R.string.passive)
            }else if(i == 2){
                touchTxt = resources.getString(R.string.steady)
            }else if (i == 3){
                touchTxt = resources.getString(R.string.vigorous)
            }
            mXYTextPaint.textSize = DeviceUtil.dip2px(context, 12f).toFloat()
            val yItemHeight = mYpos - i * mDataHeight / 3
            if (!TextUtils.isEmpty(touchTxt)){
                var textWidth = mXYTextPaint.measureText(touchTxt)
                val textHeight = getFontHeight(mXYTextPaint)
                var yHeight = yItemHeight + textHeight
                canvas.drawText(
                    "$touchTxt",
                ...
                LogUtils.i(" touchTxt $touchTxt i $i yHeight $yHeight ")
            }
    ...
    private fun drawTouchLine(canvas: Canvas) {
        ...
        if (touchItem != null && drawX > 0) {
            val linearGradient = LinearGradient(
                ...
                x,
                mOrgPoint.y.toFloat(),
                intArrayOf(0x7fD12C86, 0xD12C86),
                floatArrayOf(0.5f, 0.9f),
            ...
            touchPaint.color = Color.parseColor("#D12C86")
            canvas.drawCircle(
                drawX.toFloat(),
                drawY,
                DeviceUtil.dip2px(context, 4f).toFloat(),
            ...
            var touchTxt = ""
            if (touchItem.data <= 33) {
                touchTxt = resources.getString(R.string.passive)
            } else if (touchItem.data <= 66) {
                touchTxt = resources.getString(R.string.steady)
            } else {
                touchTxt = resources.getString(R.string.vigorous)
            }
