 ...
 * @Description:    Rvigor
 * @Author:         wb
 * @CreateDate:     2024/5/25 18:13
 * @UpdateRemark:
 */
class HeartDataView : View {
    ...
    fun init(context: Context, attrs: AttributeSet, defStyleAttr: Int) {
        ...
        touchPaint.color = Color.parseColor("#FE475A")
...
//                LogUtils.i(" top $top  curLen $curMaxLen dataItem ${dataItem?.data}")
    ...
    private fun drawBar(canvas: Canvas) {
        ...
        for (i in 0 until 24) {
            ...
            val centerX = (mXpos + barWidth * (i + 1) - barWidth / 2f).toInt()
            val rectF = RectF(
                (centerX - mBarWidth / 2).toFloat(), top.toFloat(),
                (centerX + mBarWidth / 2).toFloat(), mYpos.toFloat()
            )
...
//            LogUtils.i(" barWidth $mBarWidth mXpos $mXpos centerX $centerX")
            ...
            val rect = Rect(
                (centerX - mBarWidth / 2).toInt(), (top + mBarWidth / 2).toInt(),
                (centerX + mBarWidth / 2).toInt(), mYpos.toInt()
            )
...
//            mPaint.setShader(null)
            ...
            if (datapoints.size > 0) {
                ...
                for (i in datapoints.indices) {
                    ...
                    if (i + 1 < datapoints.size && isDrawPoint(
                            mDataList.get(i),
                            mDataList.get(i + 1)
                        )
                    ) {
...
//    }
    ...
    private fun drawShiLine(canvas: Canvas) {
        ...
        mXLinePaint.pathEffect = null
        mXLinePaint.color = Color.parseColor("#FE475A")
        if (mTimeMode == SpecDateSelectedView.TimeMode.Day) {
            val linearGradient = LinearGradient(
                x,
                0f,
                x,
                mOrgPoint.y.toFloat(),
                Color.parseColor("#22FE475A"),
                Color.parseColor("#FE475A"),
                Shader.TileMode.CLAMP
            )
            ...
            canvas.drawLine(
                ...
                0f,
                x,
                mOrgPoint.y.toFloat(), mXLinePaint
            )
        } else {
            val linearGradient = LinearGradient(
                x,
